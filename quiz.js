...
function checkcorrectAnswer(correctAnswer) {
    if (myQuestions[currentQ].correctAnswer == myQuestions[currentQ].choices[correctAnswer.id]) {
        score += 5;
...
function renderQuestion() {
    ...
    for (i = 0; i < answersEl.children.length; i++) {
        answersEl.children[i].children[0].textContent = `${(i + 1)}: ${myQuestions[currentQ].choices[i]}`;
    }
}
...
correctAnswersEl.addEventListener("click", function (e) {
    if (e.target.matches("button")) {
        checkcorrectAnswer(e.target);
        nextQuestion();
    }
});
